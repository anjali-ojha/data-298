...
import home, account, about, sample, generator
...
class MultiApp:
    ...
    def run(self):
        load_css(pathlib.Path(f'{assets_path}/style.css'))
        # app = st.sidebar(
        with st.sidebar:
            # st.sidebar.image('https://i.gifer.com/embedded/download/VYY.gif')
            st.sidebar.image(f'{assets_path}/images/logo_new.png')
            app = option_menu(
                menu_title='Options',
                options=['Home', 'Try-First-Video', 'Sample Videos', 'About', 'Account', ], # 'Tune-A-Video', 'CogVideoX-5b', 'ZeroScope', 'Text-To-Vide Zero', 'Video-Crafter', 'Edit-Pix-2-Pix'
        ...
        #
        if app == "Try-First-Video":
            generator.app()
        if app == 'Sample Videos':
            sample.app()
        if app == 'About':
            about.app()
        if app == "Account":

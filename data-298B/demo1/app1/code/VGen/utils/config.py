...
class Config(object):
    def __init__(self, load=True, cfg_dict=None, cfg_level=None, cfg_path='configs/tft2v_t2v_infer.yaml'):
        self._level = "cfg" + ("." + cfg_level if cfg_level is not None else "")
        if load:
            self.args = self._parse_args(cfg_path)
